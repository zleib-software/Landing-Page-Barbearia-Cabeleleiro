...
export function BookingModal({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const message = formatBookingMessage({
      ...
      period: period === "Proximo disponivel" ? "Mais rápido possível hoje" : period,
  ...
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-lg p-6 sm:p-8 bg-midnight-900 border border-teal-500/30 rounded-2xl shadow-2xl z-50 text-white animate-in zoom-in-95 fade-in duration-200 focus:outline-none max-h-[90vh] overflow-y-auto">
          <div className="flex items-center justify-between pb-4 border-b border-teal-500/15 mb-6">
            <div>
              <span className="text-[11px] uppercase tracking-widest font-semibold text-teal-400">
            ...
            <Dialog.Close className="p-2 rounded-lg text-ice-400 hover:text-white hover:bg-white/5 transition-colors">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider text-ice-300 mb-1.5">
                Seu Nome
              </label>
              <input
                type="text"
                ...
                required
                className="w-full bg-midnight-950 border border-teal-500/20 focus:border-teal-400 rounded-lg px-4 py-3 text-sm text-white focus:outline-none transition-all placeholder:text-ice-500"
              />
            </div>

            <div>
              <label className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wider text-ice-300 mb-1.5">
                <FaScissors className="w-3.5 h-3.5 text-teal-400" />
              ...
              <select
                ...
                required
                className="w-full bg-midnight-950 border border-teal-500/20 focus:border-teal-400 rounded-lg px-4 py-3 text-sm text-white font-medium focus:outline-none transition-all"
              >
            ...
            <div>
              <label className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wider text-ice-300 mb-1.5">
                <FaUser className="w-3.5 h-3.5 text-teal-400" />
                Especialista
              </label>
              <select
                value={professional}
            ...
            <div>
              <label className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wider text-ice-300 mb-1.5">
                <FaClock className="w-3.5 h-3.5 text-teal-400" />
            ...
            >
              ...
              <span>Enviar Pedido de Agendamento no WhatsApp</span>
            </button>
