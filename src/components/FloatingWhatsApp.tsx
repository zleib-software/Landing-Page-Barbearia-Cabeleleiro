...
export function FloatingWhatsApp() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
      ...
      {isOpen && (
        <div className="absolute bottom-20 right-0 w-[340px] sm:w-[360px] max-w-[calc(100vw-2.5rem)] bg-white dark:bg-dark-900 border border-gold-500/40 dark:border-gold-500/30 rounded-3xl shadow-2xl overflow-hidden flex flex-col animate-in fade-in slide-in-from-bottom-5 duration-200">
          {/* Header do Chat */}
          ...
          <div className="p-4 bg-light-100 dark:bg-dark-950 space-y-3">
            <div className="bg-white dark:bg-dark-800 border border-light-300 dark:border-white/10 rounded-2xl rounded-tl-none p-3.5 text-xs text-light-900 dark:text-gray-200 leading-relaxed shadow-sm">
              👋 Olá! Seja bem-vindo(a) à <strong>LUMEN & CO.</strong> Como podemos te atender hoje?
            </div>

            <div className="space-y-2 pt-1">
              <button
                onClick={() => handleQuickOption("Gostaria de saber os horários disponíveis para hoje")}
              ...
              <button
                onClick={() => handleQuickOption("Quero agendar um Corte + Barboterapia")}
                className="w-full text-left p-2.5 rounded-xl bg-gold-500/10 hover:bg-gold-gradient text-gold-800 dark:text-gold-400 hover:text-dark-950 font-semibold text-xs border border-gold-500/30 transition-all shadow-sm"
              >
                ✂️ Agendar Corte + Barboterapia
              </button>
              <button
                onClick={() => handleQuickOption("Gostaria de saber a tabela completa de valores")}
                className="w-full text-left p-2.5 rounded-xl bg-gold-500/10 hover:bg-gold-gradient text-gold-800 dark:text-gold-400 hover:text-dark-950 font-semibold text-xs border border-gold-500/30 transition-all shadow-sm"
              >
                💰 Tabela completa de preços
              </button>
              <button
                onClick={() => handleQuickOption("Gostaria de falar diretamente com a recepção")}
                className="w-full text-left p-2.5 rounded-xl bg-gold-500/10 hover:bg-gold-gradient text-gold-800 dark:text-gold-400 hover:text-dark-950 font-semibold text-xs border border-gold-500/30 transition-all shadow-sm"
              >
      ...
      <button
        ...
        className="w-16 h-16 rounded-full bg-wa hover:bg-wa-light text-white flex items-center justify-center shadow-wa-glow animate-wa-pulse hover:scale-110 active:scale-95 transition-transform relative"
        aria-label="Abrir assistente WhatsApp"
      >
        <MessageCircle className="w-8 h-8 fill-white" />
        {hasBadge && (
          <span className="absolute top-0 right-0 w-5 h-5 rounded-full bg-red-500 border-2 border-white dark:border-dark-950 text-[10px] font-extrabold flex items-center justify-center text-white">
            1
          </span>
        )}
      </button>
